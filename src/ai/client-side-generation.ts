...
const generateImageWithPrompt = async (prompt: string, apiKey: string, inputImage?: string): Promise<string> => {
    ...
    if (!response.ok) {
        ...
        throw new Error(errorBody.error?.message || 'Image generation failed on the client.');
    }

    const data = await response.json();
    const generatedContent = data.candidates?.[0]?.content?.parts?.find((p: any) => p.inlineData);

    if (!generatedContent) {
        throw new Error('No image was generated by the API.');
    }
    
    const { mimeType, data: base64Data } = generatedContent.inlineData;
